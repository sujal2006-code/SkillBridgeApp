...
export const BottomNav: React.FC<BottomNavProps> = ({ currentScreen, onNavigate }) => {
  ...
  const isMyTeamActive = currentScreen === 'my-team';
  const isAdminActive = currentScreen === 'admin';

  return (
    <nav className="md:hidden fixed bottom-0 w-full z-50 flex justify-around items-center px-1 py-2 bg-white border-t border-[#e0e3e5] shadow-lg">
      {/* Home / Dashboard */}
      <button
      ...
      >
        ...
        <span className="text-[10px] font-medium mt-0.5">Internships</span>
      </button>
      ...
      >
        ...
        <span className="text-[10px] font-medium mt-0.5">Builder</span>
      </button>
      ...
      >
        <span 
          className="material-symbols-outlined text-[22px]"
          style={{ fontVariationSettings: isMyTeamActive ? "'FILL' 1" : "'FILL' 0" }}
        >
          diversity_3
        </span>
        <span className="text-[10px] font-medium mt-0.5">My Team</span>
      </button>
